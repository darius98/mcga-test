...
#include "mcga/test_ext/matchers/functional.hpp"
...
TEST_CASE(functional, "Matchers extension: functional") {
    group("Lambdas", [] {
        ...
        test("throwsA matcher matches lambda throwing subtype", [] {
            EXPECT_MATCHER_FAILS([] { throw range_error("Some range error."); },
                                 throwsA<runtime_error>());
        });
        ...
        test(
          "throwsA matcher does not match lambda throwing different types", [] {
